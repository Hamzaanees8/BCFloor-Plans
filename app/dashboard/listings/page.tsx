  ...
  const filteredListings = listingsData
    .filter((listing) => {
      ...
      const matchesOrderId =
        (listing.id != null && String(listing.id).includes(search)) ||
        listing.orders?.some(
          (o) =>
            (o.id != null && String(o.id).includes(search)) ||
            (o.uuid != null && String(o.uuid).toLowerCase().includes(search)),
        );
    ...
    return (
      <div className="font-alexandria pb-16">
        ...
        <div className="p-4 space-y-2.5 bg-gray-50 border-b">
          <Input
            placeholder="Search by property address or Order ID"
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Select
              value={filterTour}
              onValueChange={(value) => setFilterTour(value)}
            >
              <SelectTrigger className="h-9 bg-white text-xs">
    ...
    },
    {
      ...
      cell: ({ row }: { row: Row<Listings> }) => {
        ...
        if (isPublished && latestOrder) {
          const addressSlug = listing.suite
            ? `${listing.suite} - ${listing.address}`
            : listing.address;
              ...
              return (
                <div className="text-[15px] font-[400] text-[#7D7D7D] flex justify-between items-center gap-2">
                  <Switch
                    ...
                    className={`${
                      listing.status ? "!bg-[#6BAE41]" : "!bg-[#E06D5E]"
