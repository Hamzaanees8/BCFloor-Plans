...
export default function FilePreviewModal({
  ...
  const existingGroups = Array.from(
    new Set(filesData?.files?.map((f) => f.group).filter(Boolean) || []),
  ) as string[];
  ...
  const handleAdd = () => {
    const filesToAdd = localFiles.map((file, index) => {
      ...
      return {
        ...
        is_admin_approved:
          userType === "admin" ? true : reviewFilesEnabled ? false : true,
        is_show: true,
      };
    });

    setSelectedFiles((prev: SelectedFiles[]) => [...prev, ...filesToAdd]);
            ...
            <div key={idx} className="flex gap-[10px] pr-[10px]">
              ...
              <div className="w-full flex flex-col gap-[10px]">
                ...
                {type !== "floor_plans" ? (
                        ...
                        {allSuggestions
                          .filter(
                            (item) =>
                              ...
                              item
                                .toLowerCase()
                                .includes(mediaTypes[idx].toLowerCase()),
                          )
                          .map((item, i) => (
                            <div
                              key={i}
                  ...
                  <Select
                    onValueChange={(val) =>
                      setMediaTypes((prev) => ({ ...prev, [idx]: val }))
                    }
                  >
                ...
                <div className="text-[13px] text-[#7d7d7d] grid grid-cols-3">
                  <p className="text-left col-span-1 text-[#8E8E8E] mt-1 truncate w-full overflow-hidden">
                    {file.name}
                  </p>
