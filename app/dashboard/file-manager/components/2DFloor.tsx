...
const Service: React.FC<Props & { onSave?: () => void }> = ({
    ...
    setFileItems((current) => {
      ...
      currentServiceFiles.forEach((apiFile, index) => {
        ...
        if (existing) {
          ...
          newFileItems.push({
            ...
            status: "uploaded",
            order:
              apiFile.sort_order !== undefined ? apiFile.sort_order : index + 1,
            originalData: apiFile,
          });
        }
      });
      ...
      contextualLocal.forEach((localFile) => {
        const existing = current.find(
          (item) => item.originalData?.file === localFile.file,
        );
        if (existing) {
          newFileItems.push({
            ...existing,
            ...
            order:
              localFile.sort_order !== undefined
                ? localFile.sort_order
                : existing.order,
          });
        } else {
          newFileItems.push({
  ...
  const currentSlot = orderData?.slots?.find(
    (slot) => slot.service_id === currentService?.id,
  );
  ...
  const handleImageClick = useCallback(
    (imageUrl: string | File, file: SelectedFiles | Files) => {
      if (
        ...
        !("is_complimentary" in file && file.is_complimentary)
      ) {
    ...
    (item: FileItem, isDragging?: boolean) => {
      ...
      if (isLocal) {
        displayType =
          file.type === "photo"
            ? "UnBranded Floor Plan"
            : file.type || "UnBranded Floor Plan";
      } else {
        displayType =
          file.group ||
          (file.type === "photo"
      ...
      return (
        <div
          className={`justify-self-center group w-full ${isDragging ? "opacity-80 scale-105" : ""}`}
        >
          <div>
            <p
            ...
            >
              {displayType}
              {file.is_hidden && (
                <span className="ml-2 bg-red-600 text-white text-[10px] px-1.5 py-0.5 rounded-full uppercase font-bold">
                  Hidden
                </span>
              )}
            </p>
                  ...
                  <TooltipTrigger asChild>
                    ...
                    >
                      {isLocal || file.is_admin_approved ? (
                        <>
                          <div
                            className={`w-4 h-4 border rounded mr-1.5 flex items-center justify-center ${file.is_admin_approved ? `${userType}-bg ${userType}-border` : "bg-white border-[#7D7D7D]"}`}
                          >
          ...
          <div
            ...
            style={{
              backgroundColor: `var(--${userType}-page-bg, #ffffff)`,
              fontSize: "14px",
            }}
          >
            <p
              className="col-span-2 text-[#8E8E8E] mt-1 truncate"
              style={{ fontSize: "14px" }}
              title={`Uploaded by: ${vendorName}`}
            >
              {isLocal
            ...
            <div
              className="col-span-2 flex items-center justify-between overflow-hidden"
              style={{ fontSize: "14px" }}
            >
              <p
                className="text-[#8E8E8E] mt-1 truncate pr-1"
                style={{ fontSize: "14px" }}
              >
                {isLocal
              ...
              {isLocal ? null : canDownloadFile({
                ...
                <span
                  ...
                  style={{
                    width: imagesPerRow >= 6 ? "16px" : "24px",
                    height: imagesPerRow >= 6 ? "16px" : "24px",
                  }}
                >
  ...
  return (
    <div>
        ...
        >
              ...
              {!isScrolled && (
                <span className="text-[12px] text-[#7D7D7D]">
                  {bookingToUse?.option?.title ||
                    `${bookingToUse?.option?.quantity || 0} Files`}
                </span>
              )}
            </p>
          </div>
          <div className="flex items-center gap-x-2 shrink-0">
              ...
              {!isHidingMode && userType === "vendor" && reviewFilesEnabled && (
                <Button
                  ...
                  className={`${mediaUploaded ? "bg-[#6BAE41] hover:bg-[#7dc94f]" : `${userType}-bg hover-${userType}-bg`} flex justify-center items-center font-alexandria transition-all duration-300 ${
                    isScrolled
                      ? "h-[24px] min-w-[100px] w-fit px-2 text-[10px]"
                      : "h-[26px] min-w-[120px] text-[10px] md:h-[32px] md:min-w-[150px] w-fit px-2 md:px-4 md:text-[12px]"
                  }`}
                >
                  {isSubmitting ? (
            ...
            <FilePreviewModal
              ...
              files={files}
              setSelectedFiles={setFloorFiles}
      ...
      {!isListing && (
        <div
          className={`p-3 md:p-4 flex flex-row flex-wrap justify-between items-center gap-x-4 gap-y-3 border-b border-gray-200 font-alexandria`}
        >
          ...
          <div className="flex items-center gap-4 md:gap-8 flex-wrap w-auto justify-end">
            <div className="flex flex-col items-center">
              ...
              <span className="text-[11px] md:text-[12px] text-[#666666] mt-1">
                Total Files
              </span>
            </div>
            {/* {userType !== 'vendor' && (
      ...
      <div className="w-full px-4 md:px-[200px] pt-6 md:pt-[54px]">
        <div className="w-full px-2 md:px-[80px] pb-8 md:pb-[60px] gap-y-6">
          <p className={`font-semibold text-lg ${userType}-text uppercase`}>
            Square Footage
          </p>
          ...
          {userType !== "agent" && (
            <div className="flex items-center justify-end pt-6">
              <Button
                ...
                className={`w-[150px] md:w-[143px] h-[32px] md:h-[32px]  justify-center rounded-[6px] font-raleway border-[1px] ${userType}-border ${userType}-bg text-[14px] md:text-[16px] font-[600] text-[#EEEEEE] flex gap-[5px] items-center hover:text-[#fff] hover-${userType}-bg`}
              >
                Edit
              </Button>
            </div>
