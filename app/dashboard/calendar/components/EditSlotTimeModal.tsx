...
export const EditSlotTimeModal: React.FC<EditSlotTimeModalProps> = ({
    ...
    const [propertyTimezone, setPropertyTimezone] = useState<string | undefined>(undefined);
    ...
    const globalService = useMemo(() => {
        ...
        return serviceData.find((s) => String(s.id) === String(event.service_id) || s.uuid === (event.service_id as unknown as string)) || null;
    }, [event, serviceData]);

    const currentVendor = useMemo(() => {
        if (!event || !vendorData) return null;
        return vendorData.find((v) => v.uuid === event.vendor_id) || null;
    }, [event, vendorData]);

    const matchedOrderService = useMemo(() => {
        if (!currentOrder || !event) return null;
        return currentOrder.services?.find((os) => String(os.service_id) === String(event.service_id) || os.service?.uuid === (event.service_id as unknown as string)) || null;
    }, [currentOrder, event]);

    const matchedSlot = useMemo(() => {
        if (!currentOrder || !event) return null;
        return currentOrder.slots?.find((s) => String(s.service_id) === String(event.service_id)) || null;
    }, [currentOrder, event]);
    ...
    useEffect(() => {
        ...
        const fetchTimezone = async () => {
            try {
                const loc = await getPropertyTimezone(currentOrder.property_address || '');
    ...
    }, [open, currentOrder, orderData, context]);
    ...
    }, [event, open, globalService, dateStr, hasInitialized]);
    ...
    const externalBookedSlots = useMemo(() => {
        ...
        const slots = orderData
            .filter((o) => o.uuid !== event.order_id)
            .flatMap((o) => {
                return (o.slots || []).map((s) => {
                    ...
                    const vendorObj = vendorData.find(
                        (v) => String(v.id) === String(s.vendor_id) || v.uuid === String(s.vendor_id)
                    );
                    return {
                        ...s,
                        order_id: o.id,
                        ...
                        agent_name: o.agent ? `${o.agent.first_name} ${o.agent.last_name}` : "N/A",
            ...
            .filter((s) => s.date === dateStr && String(s.vendor_id) === String(event.vendor_id));
        return slots.map((s) => ({
            ...s,
    ...
    }, [event, orderData, dateStr, serviceData, vendorData]);
    ...
    const adjustEndTime = (minutes: number) => {
        const parsedStart = parse(`2000-01-01 ${startTime}`, 'yyyy-MM-dd HH:mm:ss', new Date());
        const parsedEnd = parse(`2000-01-01 ${endTime}`, 'yyyy-MM-dd HH:mm:ss', new Date());
    ...
    const handleSave = async () => {
        if (!event || !currentOrder || !globalService || !matchedSlot || !matchedOrderService) {
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
            <DialogContent className="max-w-3xl p-6 font-alexandria bg-white rounded-lg shadow-2xl border border-gray-100 overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 bg-gray-50/50 p-4 rounded-lg border border-gray-100/50 text-[13px]">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 text-gray-600">
                            <span className="font-semibold text-gray-800 min-w-[100px]">Order ID:</span>
                        ...
                        <div className="flex items-center gap-2 text-gray-600">
                            ...
                            <span className="font-semibold text-gray-800 min-w-[80px]">Agent:</span>
                            <span>
                                {currentOrder.agent
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 text-gray-600">
                            ...
                            <span className="font-semibold text-gray-800 min-w-[80px]">Date:</span>
                            <span>{dateStr}</span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-600">
                            <span className="font-semibold text-gray-800 min-w-[100px]">Service:</span>
                            <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-xs font-medium border border-blue-100">
                                {globalService.name}
                            </span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-600">
                            <span className="font-semibold text-gray-800 min-w-[100px]">Vendor:</span>
                            <span>
                                {currentVendor
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 bg-gray-50/20 border border-gray-100 p-4 rounded-lg mb-4 text-[13px]">
                ...
                <div className="mx-6 mb-6">
                    <div className="border border-gray-200 rounded-lg overflow-hidden bg-gray-50/10 h-[350px] relative">
                        <OneDayCalendar
                            selectedVendors={event.vendor_id ? [event.vendor_id] : []}
                            service={{
                            ...
                            className="h-full border-none shadow-none mt-0"
                        />
                    </div>
                </div>
