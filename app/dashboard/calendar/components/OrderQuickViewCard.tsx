...
import CancelOrderDialog, {
  CancelPreviewData,
} from "../../orders/components/CancelOrderDialog";
...
export default function OrderQuickViewCard({
  data,
  onClose,
  vendorData,
  serviceData,
  ...
  const [showCancelServiceDialog, setShowCancelServiceDialog] =
    React.useState(false);
  const [cancelServicePreviewData, setCancelServicePreviewData] =
    React.useState<CancelPreviewData | null>(null);
  const [isCancelServiceLoading, setIsCancelServiceLoading] =
    React.useState(false);
  const [cancelTargetService, setCancelTargetService] = React.useState<{
    uuid: string;
    name: string;
  } | null>(null);
  ...
  const allNotes = Array.isArray(CurrentOrder?.notes)
    ? CurrentOrder.notes
    : typeof CurrentOrder?.notes === "string"
      ? JSON.parse(CurrentOrder.notes)
      : [];

  const agentNotes = allNotes.filter(
    (note: any) =>
      note.internal === "false" ||
      note.is_internal === false ||
      (!note.internal && !note.is_internal),
  );
  const internalNotes = allNotes.filter(
    (note: any) => note.internal === "true" || note.is_internal === true,
  );
  ...
  if (CurrentOrder?.property?.uuid)
    queryParams.append("listingId", CurrentOrder.property.uuid);
  if (CurrentService?.uuid)
    queryParams.append("serviceId", CurrentService.uuid);
  ...
  const handleCancelServiceClick = async () => {
    ...
    try {
      ...
      setCancelServicePreviewData({
        ...
        fee_percentage: 0,
        booking_datetime:
          CurrentOrder.slots?.[0]?.date || new Date().toISOString(),
        deadline: "",
        timezone: "",
  ...
  const handleCancelService = async (reason?: string) => {
    ...
    const statusUpper = (CurrentOrder.payment_status || "")
      .toUpperCase()
      .trim();
    ...
    if (
      ...
      toast.error(
        "You cannot cancel a paid order. You can only refund the order.",
      );
      return;
    }
    ...
    >
      <CardContent className="flex flex-col gap-[12px] p-0 custom-scrollbar">
        ...
        <div className="space-y-2 text-sm overflow-auto custom-scrollbar">
          <p
            className={`text-[15px] font-[400] leading-[32px] ${userType}-text`}
          >
            ...
            {userType === "admin" ? (
              ...
              >
                {CurrentOrder?.agent?.first_name}{" "}
                {CurrentOrder?.agent?.last_name}
              </Link>
            ) : (
              <span>
                {CurrentOrder?.agent?.first_name}{" "}
          ...
          <div className="flex items-center space-x-[18px]">
            ...
            <a
              className={`hover:underline text-[15px] font-[400] leading-[32px] ${userType}-text`}
              href={`https://www.google.com/maps?q=${CurrentOrder?.property_address + "," + CurrentOrder?.property_location}`}
              target="_blank"
          ...
          <div className="flex  items-start space-x-[18px]">
            <Calendar
              className="w-[24px] mt-[7px] text-[#666666]"
              strokeWidth={1}
            />
          ...
          {userType === "admin" ? (
            <>
              <div className="grid grid-cols-1 gap-y-[0px]">
                <span className="text-[#8E8E8E] text-[10px] font-[700]">
                  Notes viewable by agent
                </span>
                {agentNotes.length > 0 ? (
                  agentNotes.map((note: { note: string }, idx: number) => (
                    <span
                      key={idx}
                      className="text-[15px] font-[400] text-[#666666] leading-[32px]"
                    >
                      {note.note}
                    </span>
                  ))
                ) : (
                  <span className="text-[15px] font-[400] text-[#8E8E8E] italic leading-[32px]">
                    No agent notes
                  </span>
                )}
              </div>
              <div className="grid grid-cols-1 gap-y-[0px] mt-2">
                <span className="text-[#8E8E8E] text-[10px] font-[700]">
                  Internal notes not viewable by agent
                </span>
                {internalNotes.length > 0 ? (
                  internalNotes.map((note: { note: string }, idx: number) => (
                    <span
                      key={idx}
                      className="text-[15px] font-[400] text-[#666666] leading-[32px]"
                    >
                      <span className="text-[10px] bg-[#E06D5E] text-white px-2 py-0.5 rounded-full uppercase mr-2 relative -top-0.5">
                        Internal
                      </span>
                      {note.note}
                    </span>
      ...
      <CardFooter className="p-0 mt-10">
        <div className="w-full grid grid-cols-2 gap-[10px]">
          <Link
            ...
            className={`${userType}-bg border-[1px] text-[14px] flex justify-center items-center ${userType}-border text-[#fff] rounded-none w-full h-[32px] hover:text-white hover:brightness-110`}
          >
            Manage Media
          </Link>
          <Button
          ...
          {showCancelButton && (
            <Button
              ...
              className={`bg-transparent border-[1px] text-[14px] flex justify-center items-center ${userType}-border text-red-500 hover:text-white rounded-none w-full h-[32px] hover:bg-red-500 hover:border-red-500`}
            >
      ...
      {showSwapModal && CurrentOrder && CurrentService && (
