...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  .map(group => ({
    ...
    items: group.items.filter(item => {
      ...
      if (userType === "agent" && item.url === "/dashboard/global-settings") {
        item.title = "Settings";
      }
      ...
      <SidebarContent className="bg-[#E4E4E4] pt-[20px] px-[25px] sidebar-scroll">
        <div >
          {showBackButton ? (
            ...
            <div className="h-[32px]" /> // Invisible placeholder
          )}
        </div>
                    ...
                    return (
                      <SidebarMenuItem key={subItem.title}>
                        <SidebarMenuButton
                          asChild
                          isActive={isActive}
                          className={`text-[16px] font-normal ${isActive ? `${userType}-text !font-bold  data-[active=true]:bg-transparent` : "text-[#7D7D7D] hover:text-[#7D7D7D]"}`}
                        >
                          <Link href={subItem.url} className={`text-[16px] flex items-center gap-2 font-normal ${isActive ? "!text-[#4290E9] !font-bold" : "text-[#7D7D7D]"}`}>
                            {subItem.icon && (
                              <subItem.icon
                                className={`h-4 w-4 ${isActive ? `${userType}-text bg-none` : "text-[#7D7D7D] "}`}
                              />
                            )}
                            {subItem.title}
                          </Link>
