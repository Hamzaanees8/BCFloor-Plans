...
const ServiceItem = ({ selectedService, servicesData, index, onChange, currentUser, fieldErrors }: ServiceItemProps) => {
    const [showTimeFields, setShowTimeFields] = useState(false);
    const { userType } = useAppContext();

    // Find the full service data
    const serviceData = servicesData.find(s => s.uuid === selectedService.service_id);
    ...
    const getOptionData = (optionUuid: string) => {
        return selectedService.options.find(opt => opt.option_uuid === optionUuid);
    };
        ...
        <div className="p-4 w-[450px] text-[#666] font-alexandria">
            ...
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center justify-between gap-3 w-full">
                    <span
                        className={`w-[400px] border rounded-[8px] h-[42px] flex items-center pl-2 ${fieldErrors?.[`services[${index}].service_id`] ? 'border-red-500' : 'border-[#BBBBBB]'}`}
            ...
            {fieldErrors?.[`services[${index}].service_id`] && (
                <p className="text-red-500 text-xs mt-1 mb-2">{fieldErrors[`services[${index}].service_id`][0]}</p>
            )}
                                    ...
                                    <AccordionContent className="p-4 text-[#666]">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                />
                                                {hasError && (
                                                    <p className="text-red-500 text-xs mt-1">{fieldErrors[priceErrorKey][0]}</p>
                                                )}
                                            </div>
                                            <div>
            ...
            <div className="flex justify-end mr-9 mt-4">
                <Button
                    ...
                    onClick={() => handleRemove(selectedService?.vendor_service_id ?? '')}>Remove</Button>
            </div>
            {showTimeFields && productOptions.length === 0 && (
